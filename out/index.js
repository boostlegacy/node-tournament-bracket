...
__export(require("./components/team"));
__export(require("./components/node"));
function nextChar(c) {
    let returnString = '';
...
class Bracket {
    constructor({ type = "Single", teams = [], best_of = 1 } = {}) {
        this.type = type;
    ...
    set_round_best_of(round, best_of) {
        let rnd = this.get_round(round);
        if (rnd instanceof Error) {
            return rnd;
        }
    ...
    update_bracket_nodes_and_seeding() {
        ...
        while (Math.pow(2, total_rounds) < this.team_count) {
            total_rounds++;
        }
        ...
        var upper_team_count = Math.pow(2, (this.rounds - 1));
        ...
        if (this.type === "Single") {
            ...
            let recurse = (depth, current_node, upper_team) => {
                ...
                let lower_team = teams_sorted[Math.pow(2, depth) - upper_team.seed];
                current_node.team_lower = lower_team;
                depth++;
                if (depth < this.rounds || (lower_team_count === 0 && depth === this.rounds)) {
                    if (upper_team.seed <= Math.pow(2, (depth - 1)) / 2) {
                    ...
                    if (lower_team.seed >= Math.pow(2, (depth - 1)) / 2) {
            ...
            if (this.team_count < Math.pow(2, this.rounds)) {
